...
function conteudoPopup(municipio) {
  ...
  if (folhaInfo) {
    ...
    const notaAno = folhaInfo.ano !== anoSelecionado
      ? `<div class="detalhe-discreto">Sem auditoria de folha para ${anoSelecionado}; mostrando a mais recente disponível (${folhaInfo.ano}).</div>`
      : "";

    blocoFolha = `
      <div class="obs-box" style="background:${corFundo};">
        <b>Folha de pagamento ${folhaInfo.ano}:</b>
...
//    respeitando o ano em tela — ver folhaNoAnoOuAnterior)
...
function observacaoResumida(municipio) {
  const folhaInfo = folhaNoAnoOuAnterior(municipio, anoSelecionado);
  if (folhaInfo && folhaInfo.folha.observacoes) {
    return folhaInfo.folha.observacoes;
  }
  if (municipio.obs_geral) {
